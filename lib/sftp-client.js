...
const sftp_commands = [
    ...
    ['readdir', 'ls', 'list current directory'],
...
const sftp_call_schema = {
    ...
    lcd: {
        ...
        resolve: (pars, cb) => {
            let dir = pars[0];
            if (!curr_local_path && !path.isAbsolute(dir)) {
                cb('The current local path is not specified.');
            } else {
                if (dir === '.') {
                    pars[0] = curr_local_path;
                } else if (!path.isAbsolute(pars[0])) {
                    pars[0] = path.join(curr_local_path, pars[0]);
                }
                cb(undefined, pars);
            }
        },
    ...
    readdir_: {
        ...
        resolve: (pars, cb) => {
            if (!curr_remote_path && pars[0].indexOf('/') !== 0) {
                cb('The current remote path is not specified.');
            } else {
                if (pars[0] === '.') {
                    pars[0] = curr_remote_path;
                } else if (pars[0] === '..') {
                    let old_path = pars[0];
                    let pos = curr_remote_path.lastIndexOf('/');
                    if (pos !== -1) {
                        pars[0] = curr_remote_path.substr(0, pos);
                    } else {
                    ...
                    let old_path = pars[0];
                    while (pars[0].indexOf('..') === 0) {
                        let pos = curr_dir.lastIndexOf('/');
                        if (pos === -1) {
                            return cb('Invalid path: ' + old_path);
                        }
                        pars[0] = pars[0].length > 3 ? pars[0].substr(3) : '';
                        curr_dir = curr_dir.substr(0, pos);
                    }
                    pars[0] = curr_dir + '/' + pars[0];
                }
                //console.log(pars);
                cb(undefined, pars);
            }
            ...
            if (hits.length === 0) {
                ...
                if (!schema.isLocal) {
                    let call = (params) => {
                        ...
                        return Promise.all(keys.map((s, i) => {
                            return new Promise((resolve, reject) => {
                                rl.question(schema[s].description + ' = ', (answer) => {
                                    params.push(answer);
                                    resolve();
                                });
                            });
