...
const
    ...
    local_fsys = require('./local-filesystem');
...
const sftp_call_schema = {
    ...
    lcd: {
        path: {
            description: chalk.bold('local path')
        },
        isLocal: true,
        resolve: (pars, cb) => {
    ...
    readdir_: {
        path: {
            description: chalk.bold('remote path')
        },
        resolve: (pars, cb) => {
    ...
    fastGet: {
        remotePath: {
            description: chalk.bold('remote file path')
        },
        localPath: {
            description: chalk.bold('local file or directory path')
        },
        resolve: (pars, cb) => {
            resolve_remote_path(pars[0], (err, _path) => {
                if (err)
    ...
    fastPut: {
        localPath: {
            description: chalk.bold('local file path'),
            required: true,
        },
        remotePath: {
            description: chalk.bold('remote file or directory path')
        },
        resolve: (pars, cb) => {
    ...
    rl.on('line', line => {
        if (line.trim() === 'exit') {
            ...
            let cmds = sftp_commands.map(c => {
                return {
                    cmd: chalk.bold(c[1]),
                    descr: chalk.yellow(c[2])
                }
            });
            cmds.push({
                cmd: chalk.bold('"tab"'),
                descr: chalk.yellow('completes the input, if possible')
            }, {
                cmd: chalk.bold('?'),
                descr: chalk.yellow('show help information')
            });
            console.table(cmds);
            rl.prompt();
        } else if (line) {
            ...
            if (hits.length === 0) {
                console.log(chalk.red('command "%s" is not known ...'), line.trim());
                rl.prompt();
            } else if (hits.length > 1) {
                console.log(chalk.gray('command "%s" is ambiguous ...'), line.trim());
                rl.prompt();
            } else {
                const method = hits[0][0]; //...
                const schema = sftp_call_schema[method];
                const keys = Object.keys(schema).filter(k => {
                    const ptype = typeof schema[k];
                    return ptype === 'object';
                });
                if (!schema.isLocal) {
                    const call = params => {
                        ...
                        return input(0).then(() => {
                            schema.resolve(params, (err, params) => {
                                if (err) {
                                    ...
                                    const _method = method.replace(/_+$/g, '')
                                    sftp[_method](...params, (err, result) => {
                                        if (err) {
                                            console.log(err.message || err);
                                        } else if (result) {
                                            schema.callback(...params, result);
                                        }
                                        console.log(`${_method} done`);
                                        rl.prompt();
                                    });
                                }
                            })
                    ...
                    switch (method) {
                        case 'pwd':
                            if (curr_remote_path) {
                                console.log('    ' + chalk.green(curr_remote_path));
                            } else {
                                console.log('    ' + chalk.red('not yet specified'));
                            }
                            rl.prompt();
                            break;
                        case 'lpwd':
                            console.log('    ' + chalk.green(curr_local_path));
                            rl.prompt();
                            break;
                        case 'lcd':
                            (() => {
