            ...
            if (answer) {
                conn.once('x11', function(info, accept, reject) {
                    ...
                    xserversock.on('connect', function() {
                        console.log(chalk.grey('connected to x11 server ...'));
                        ...
                        xserversock.on('close', () => {
                            console.log(chalk.grey('s: close'));
                            xclientsock.end();
                        });
                        xserversock.on('end', (msg) => {
                            if (msg) {
                                console.log(chalk.grey('s: end = ' + msg));
                            }
                            xclientsock.end();
                        });
                        ...
                        xclientsock.on('end', (msg) => {
                            if (msg) {
                                console.log(chalk.grey('c: end = ' + msg));
                            }
                            xserversock.end();
                        });
                        xclientsock.on('close', () => {
                            console.log(chalk.grey('c: close'));
                            xserversock.end();
                        });
                        xclientsock.on('error', (err) => {
                            console.log(chalk.grey('c: error'));
                            console.error(err);
                        });
                        rl.prompt();
                    });
