...
schema = {
    properties: {
        ...
        host: {
            ...
            required: true,
            default: process.env.SSH_CONF_NAME,
            ask: () => {
                return !process.env.SSH_CONF_NAME
            }
        },
        port: {
            ...
            default: 22,
            ask: () => {
                let host = process.env.SSH_CONF_NAME || prompt.history('host').value;
        ...
        password: {
            ...
            replace: '*',
            ask: () => {
                const host = process.env.SSH_CONF_NAME || prompt.history('host').value;
