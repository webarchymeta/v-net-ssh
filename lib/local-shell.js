...
schema = {
    properties: {
        ...
        x_server_ip: {
            ...
            default: 'localhost',
            ask: () => {
                return process.env.SHELL_APP && process.env.SHELL_APP === 'x11' && !process.env.X11_SERVER_IP || prompt.history('app').value === 'x11';
            }
        },
        x_server_port: {
            type: 'integer',
            ...
            default: 6000,
            ask: () => {
                return process.env.SHELL_APP && process.env.SHELL_APP === 'x11' && !process.env.X11_SERVER_PORT || prompt.history('app').value === 'x11';
            }
        }
    }
};
