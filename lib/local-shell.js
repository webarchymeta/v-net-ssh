...
schema = {
    properties: {
        proxy: {
            ...
            ask: () => {
                return !process.env.LOCAL_LAN && (!process.env.SOCKS5_ADDRESS || !process.env.SOCKS5_PORT);
            }
        },
        socks5_host: {
        ...
        host: {
            ...
            required: true,
            default: process.env.SSH_CONF_NAME || process.env.MDNS_HOST,
            ask: () => {
                return !process.env.SSH_CONF_NAME && !process.env.MDNS_HOST
            }
        },
        port: {
            description: chalk.hex('#aaaaaa')('port'),
            type: 'integer',
            default: process.env.MDNS_PORT ? parseInt(process.env.MDNS_PORT) : 22,
            ask: () => {
                let host = process.env.SSH_CONF_NAME ? process.env.SSH_CONF_NAME : prompt.history('host') ? prompt.history('host').value : undefined;
                return !process.env.MDNS_PORT && (!host || !hosts[host]);
            }
        },
        user: {
